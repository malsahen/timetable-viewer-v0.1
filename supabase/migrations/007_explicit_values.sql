-- Migration: Seed Teacher Mapping (Explicit Values)
...
-- Note: ON CONFLICT(id) is just a placeholder, in reality we rely on the next select or manual management.
-- But to be safe and simple, let's just make sure we HAVE a version.
...
-- 3. Insert Mappings
-- Clean up old mappings for these teachers in the current version
...
-- Insert new mappings linked to the current active teacher version
